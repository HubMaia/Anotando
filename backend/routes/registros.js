...
// Rota para filtrar registros por horário
router.get('/horario/:horario', async (req, res) => {
  try {
    const { horario } = req.params;
    const userId = req.userData.userId;

    // Lógica para buscar registros filtrados por horário
    const [registros] = await pool.query(
      'SELECT * FROM registros WHERE usuario_id = ? AND horario = ? ORDER BY data DESC',
      [userId, horario]
    );

    res.status(200).json({ registros });
  } catch (error) {
    console.error('Erro ao filtrar registros por horário:', error);
    res.status(500).json({ message: 'Erro ao filtrar registros por horário' });
  }
});
